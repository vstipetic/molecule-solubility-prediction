"""Inference script for scratch-trained Transformer model.

This script loads a trained Transformer model and runs inference on an input
...
def load_transformer_checkpoint(
    ...
    # Get config - may be stored or use defaults
...
def run_inference_transformer(
    ...
    """Run inference with a trained Transformer model.
    ...
    model, tokenizer, config = load_transformer_checkpoint(model_path, device)
    model.eval()
    ...
    dataset = TransformerSolubilityDataset(
        smiles_list, dummy_targets, tokenizer, max_length, is_huggingface=False
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run inference with trained Transformer model"
    )
    parser.add_argument(
